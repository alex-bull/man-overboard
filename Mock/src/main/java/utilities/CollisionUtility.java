...
public class CollisionUtility {
    ...
    public static boolean isPointInPolygon(MutablePoint point, List<MutablePoint> polygon) {
        ...
        boolean c = false;
        for(int i=0;i<polygon.size();j=i++)c^=polygon.get(i).getYValue()>point.getYValue()^polygon.get(j).getYValue()>point.getYValue()&&point.getXValue()<(polygon.get(j).getXValue()-polygon.get(i).getXValue())*(point.getYValue()-polygon.get(i).getYValue())/(polygon.get(j).getYValue()-polygon.get(i).getYValue())+polygon.get(i).getXValue();
        return c;
    }
